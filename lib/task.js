...
module.exports.scheduleTriggeredJob = function() {
	...
	//		contract = contracts[i];
			if(contract.exchange === 'future'){
				global.redis_client.get(ctpTrader.makeRedisKey(contract), function(err, priceInfoString) {
					var priceInfo = JSON.parse(priceInfoString);
					var currentPrice = priceInfo*100;
					schedule.scheduleJob('*/1 * * * * *', function(){
						var user_with_trigger = require('../controllers/ctpTrader').user_with_trigger;
                        console.log(user_with_trigger);
                        if(!user_with_trigger){
                            console.log('user cache empty.');
                            return;
                        }
						async.each(user_with_trigger, function(user, callback){
							if(currentPrice >= user.winPoint || currentPrice <= user.lossPoint) {
								if(user.real){
...
module.exports.scheduleHiveControlJob = function() {
    ...
    schedule.scheduleJob('0 14 9 * * *', function(){
        ctpTrader.initHive();
    });
