...
module.exports.scheduleFuturesForceCloseJob = function() {
    ...
    schedule.scheduleJob(rule, function() {
        ...
        //mongoose.connect(config.db, options);
        //var db = mongoose.connection;
        //db.on('error', console.error.bind(console, 'scheduleFuturesForceCloseJob connection error...'));
        //db.once('open', function callback() {
            console.log('goldenbull db opened for scheduleFuturesForceCloseJob');
            mockTrader.User.find({status: 0}, function (err, users) {
                console.log('scheduleFuturesForceCloseJob find user callback');
                if (err) {
