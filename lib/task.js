...
module.exports.scheduleFuturesRiskControlJob = function() {
    ...
    db.once('open', function callback() {
        schedule.scheduleJob('*/30 * * * * *', function() {
            mockTrader.User.find({status: 0}, function (err, users) {
                if (err) {
                    logger.warn('scheduleFuturesRiskControlJob error:' + err.toString());
                    return;
                }
                async.each(users, function (user, callback) {
                    mockTrader.windControl(user._id, 0, function (err) {
                        if (err) {
                            logger.warn('scheduleFuturesRiskControlJob error:' + err.toString());
                        }
                        callback(null);
                    }, function (err) {
                        //logger.info('scheduleFuturesRiskControlJob done');
                    });
                });
            });
        });
...
module.exports.scheduleFuturesForceCloseJob = function() {
    ...
    schedule.scheduleJob(rule, function() {
        ...
        /*
        ctpTrader.User.find({status: 0}, function(err, users) {
            if (err) {
                logger.warn('scheduleFuturesForceCloseJob for real error:' + err.toString());
                return;
            }
            async.each(users, function(user, callback) {
                ctpTrader.windControl(user._id, 1, function (err) {
                    if (err) {
                        logger.warn('scheduleFuturesForceCloseJob for real error:' + err.toString());
                    }
                    callback(null);
                }, function(err) {
                    logger.info('scheduleFuturesForceCloseJob for real done');
                });
            });
        });
        */
        ...
        mockTrader.User.find({status: 0}, function (err, users) {
            ...
            async.each(users, function(user, callback) {
                console.log('forceClose user', user);
                mockTrader.windControl(user._id, 1, function (err) {
                    if (err) {
                        logger.warn('scheduleFuturesForceCloseJob error:' + err.toString());
                    }
                    callback(null);
