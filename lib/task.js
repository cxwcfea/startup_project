...
module.exports.scheduleFuturesForceCloseJob = function() {
    ...
    schedule.scheduleJob(rule, function() {
        ...
        mockTrader.User.find({status: 0}, function (err, users) {
        //mockTrader.User.find({$and:[{status:0}, {real:false}]}, function(err, users) {
            if (err) {
            ...
            async.each(users, function(user, callback) {
                ...
                if (user.real == false){
                    mockTrader.windControl(user._id, 1, {exchange:'future', stock_code:'IFCURR'}, function (err) {
                        if (err) {
                            logger.warn('scheduleFuturesForceCloseJob error:' + err.toString());
                        }
                        callback(null);
                    ...
                    ctpTrader.windControl(user._id, 1, {exchange:'future', stock_code:'IFCURR'}, function (err) {
                        if (err) {
                            logger.warn('scheduleFuturesForceCloseJob error:' + err.toString());
                        }
                        callback(null);
