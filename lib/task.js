    ...
    schedule.scheduleJob('*/15 * * * * *', function() {
        mockTrader.User.find({}, function (err, users) {
            ...
            async.each(users, function (user, callback) {
				if(user.real == true){
					ctpTrader.windControl(user._id, 0, {exchange:'future', stock_code:'agCURR'}, function (err) {
						if (err) {
							logger.warn('scheduleFuturesRiskControlJob error:' + err.toString());
						}
						callback(null);
					...
					mockTrader.windControl(user._id, 0, {exchange:'future', stock_code:'agCURR'}, function (err) {
						if (err) {
							logger.warn('scheduleFuturesRiskControlJob error:' + err.toString());
						}
						callback(null);
...
module.exports.scheduleTriggeredJob = function() {
	...
	var userContract = {exchange:'future', stock_code:'agCURR'};
                    ...
                    mockTrader.Portfolio.find({userId: user.id}, function(err, portfolio) {
                        ...
                        for (var p in portfolio) {
                            ...
                            mockTrader.Contract.findOne({_id: portf.contractId}, function(err, contract) {
                                ...
                                var buy_price = portf.total_point/Math.abs(portf.quantity);
                                if(portf.quantity > 0){
                                    var trigger_in_price = buy_price + parseFloat(user.winPoint)/(contract.point_value/100)/Math.abs(portf.quantity/100) + 1;
                                    var trigger_out_price = buy_price - parseFloat(user.lossPoint)/(contract.point_value/100)/Math.abs(portf.quantity/100) + 1;
                                    if ((user.winPoint != 0 && currentPrice >= trigger_in_price) || 
                                        (user.lossPoint != 0 && currentPrice <= trigger_out_price)) {
                                    ...
                                    var trigger_in_price = buy_price - parseFloat(user.winPoint)/(contract.point_value/100)/Math.abs(portf.quantity/100) - 1;
                                    var trigger_out_price = buy_price + parseFloat(user.lossPoint)/(contract.point_value/100)/Math.abs(portf.quantity100) - 1;
                                    if ((user.winPoint != 0 && currentPrice <= trigger_in_price) || 
                                        (user.lossPoint != 0 && currentPrice >= trigger_out_price)) {
                                        console.log('triggerrrrrrrrrrrrrrrrrrr');
                                        if(user.real){
...
module.exports.scheduleFuturesForceCloseJob = function() {
    ...
    schedule.scheduleJob(rule, function() {
        ...
        mockTrader.User.find({}, function (err, users) {
            ...
            var contract = {exchange:'future', stock_code:'agCURR'};
            async.each(users, function(user, callback) {
                if (user.real == false){
...
module.exports.scheduleHiveControlJob = function() {
    ...
    schedule.scheduleJob('0 11 9 * * *', function(){
        ctpTrader.initHive(1);
    });
    schedule.scheduleJob('1 31 11 * * *', function(){
        ctpTrader.destroyHive();
    });
    schedule.scheduleJob('0 59 12 * * *', function(){
        ctpTrader.initHive(1);
    });
