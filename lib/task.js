...
module.exports.scheduleResetWechatUserJob = function() {
    ...
    schedule.scheduleJob(rule, function(){
        ...
        db.on('error', console.error.bind(console, 'connection error...'));
        db.once('open', function callback() {
            console.log('goldenbull db opened for scheduleResetWechatUserJob');
            mockTrader.User.update({'wechat.wechat_uuid':{$exists:true}}, {$set:{'wechat.logged':false}}, {multi:true}, function(err, numberAffected, raw) {
                if (err) {
