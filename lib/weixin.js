...
//var wechatAppId = 'wx182b3688b9b060f7';  // niujinwang
//var wechatSecret = '13ea8e4782476ccdf2d58bbbb1634513'; // niujinwang

var wechatAppId = 'wx1269a608e8627e89';  // xiaoniu
var wechatSecret = '5179dd37d74f4c028934613e2b483743'; // xiaoniu

//var wechatAppId = 'wx8e029a336ad0a880';   // jiaoyikuaishou
//var wechatSecret = 'a6217b36a4ef01b9f801c2d338fb7548'; // jiaoyikuaishou
...
function handleRandomText(res, inData) {
    var xmlObj = {
        xml: [
            ...
            {
                Content: {_cdata: '欢迎关注牛金网！融资相关问题请联系客服:\nQQ:4006921388\n电话:400-692-1388'}
            }
        ]
    };
    var ret = xml(xmlObj);
...
function handleSubscribe(res, inData) {
    ...
    var xmlObj = {
        xml: [
            ...
            {
                Content: {_cdata: '亲，您怎么现在才来！欢迎关注牛金网！'}
            }
        ]
    };
    var ret = xml(xmlObj);
...
function getManagerMenu() {
    var StockButton = new ComplexButton('融资炒股');
    var ButtonFreeTry = new ButtonUrl('免费体验','http://www.niujinwang.com/mobile/?refer=wechat#/exp');
    var ButtonDaysNiu = new ButtonUrl('天天牛','http://www.niujinwang.com/mobile/?refer=wechat#/ttn');
    var ButtonMonthNiu = new ButtonUrl('月月牛','http://www.niujinwang.com/mobile/?refer=wechat#/yyn');
    ...
    var ButtonInvestment = new ButtonUrl('天天投', 'http://www.niujinwang.com/mobile/?refer=wechat#/invest');
    InvestmentButton.sub_button[0] = ButtonInvestment;

    var MyAccountButton = new ComplexButton('服务中心');
    var ButtonUser = new ButtonUrl('个人中心','http://www.niujinwang.com/mobile/?refer=wechat#/user');
    var ButtonWeixin = new ButtonClick('绑定微信','binding');
    var ButtonLoginLongterm = new ButtonUrl('免登录','http://www.niujinwang.com/mobile/?refer=wechat#/login_longterm');
...
function getToken(cb) {
    ...
    global.redis_client.get('weixin_token', function(err, reply) {
        if (!err && reply) {
            ...
            needle.get('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' + wechatAppId + '&secret=' + wechatSecret, function(err, resp, body) {
                if (err) {
...
module.exports.login = function(req, res, next) {
    ...
    var auth = passport.authenticate('wechat', option, function(err, user, info) {
        if (err) {
            console.log('in callback err:' + err.toString());
            next();
        }
...
function getJSTicket(cb) {
    getToken(function(err, token) {
        ...
        global.redis_client.get('weixin_niujin_ticket', function(err, reply) {
            if (err) {
                console.log('restore weixin ticket err:' + err.toString());
                return cb(err);
            }
            if (reply) {
                console.log('restore weixin ticket ' + reply);
                cb(null, reply);
            } else {
                console.log('fetch new weixin ticket');
                needle.get('https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token=' + token + '&type=jsapi', function(err, resp, body) {
                    if (err) {
                        console.log('get Weixin ticket err:' + err.toString());
                        cb(err);
                    } else {
                        global.redis_client.set('weixin_niujin_ticket', body.ticket, redis.print);
                        global.redis_client.expire('weixin_niujin_ticket', 7100, redis.print);
                        cb(null, body.ticket);
                    }
                });
            }
...
module.exports.getJSToken = function(req, res, next) {
    getJSTicket(function(err, ticket) {
        if (err) {
            return res.status(500).send({error_msg:err.toString()});
        }
        var url = req.query.url;
        console.log('getJSToken url:' + url);
        var ts = Date.now();
        ts = parseInt(ts/1000);
        var obj = {
            noncestr: credential.code,
            jsapi_ticket: ticket,
            timestamp: ts,
            url: url,
        };
