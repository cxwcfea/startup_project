...
function handleRandomText(res, inData) {
    ...
    var xmlObj = {
        xml: [
            ...
            {
                Content: {_cdata: '欢迎关注牛金网！您好，目前该服务号仍处于开发测试阶段，无法为您提供服务，请谅解'}
            }
        ]
    };
    var ret = xml(xmlObj);
...
function getToken(cb) {
    global.redis_client.hget('weixin_hash', 'weixin_token', function(err, reply) {
        console.log('redis reply for weixin_token ' + reply);
        if (!err && reply && reply.token && reply.expires > Date.now()) {
            ...
            needle.get('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wx1269a608e8627e89&secret=5179dd37d74f4c028934613e2b483743', function(err, resp, body) {
                if (err) {
                    ...
                    global.redis_client.hset('weixin_hash', 'weixin_token', weixin_token, redis.print);
                    cb(null, weixin_token.token);
                }
            });
        }
    });
