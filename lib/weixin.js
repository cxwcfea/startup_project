...
function sendRechargeSuccessMsg(openID) {
...
function sendTemplateMsg(t_id, openID) {
    switch (t_id) {
        case '1':
            sendRechargeSuccessMsg(openID);
            break;
        case '2':
            sendAccountExpireMsg('81226058', '2015-05-15 15:00', openID);
            break;
        case '3':
            sendAccountReadyMsg('Homs', '81226053', '695920', openID);
            break;
        case '4':
            sendBalanceChangeMsg('余额提现', 200, openID);
            break;
        default :
            sendRechargeSuccessMsg(openID);
            break;
...
module.exports.sendWeixinTemplateMsg = function(userMobile, templateID) {
    User.findOne({mobile:userMobile}, function(err, user) {
        ...
        sendTemplateMsg(templateID.toString(), user.profile.weixin_id);
    });
};
