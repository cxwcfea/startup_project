            ...
            if (err) {
                ...
                if (result.xml && result.xml.FromUserName && result.xml.Event) {
                    if (result.xml.Event[0] == 'subscribe') {
						...
						var greetings  = '您好，感谢关注小牛！^_^您直接点击左下方小键盘输入想要咨询的问题即可，客服MM会在第一时间答复您。';
<<<<<<< HEAD
						var tmp = result.xml;
						customFlag = true;
						sendTextMsg(res, tmp, greetings);
=======
						sendTextMsg(res, result.xml, greetings);
			//transferCustomerService(res, result.xml);
>>>>>>> 3d41cf238e3bc3b8f8724b6081437e0ee08b228e
