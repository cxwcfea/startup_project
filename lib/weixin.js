...
function SuccessRecharge(First,AccountName,Amount,Result,Remark) {
    this.first = First;
    this.keyword1 = AccountName;
    this.keyword2 = Amount;
    this.keyword5 = Result;
    this.remark = Remark;
}

module.exports.sendTemplateMsg = sendRecharegSuccessMsg;

var sendRecharegSuccessMsg = function(req, res) {
    ...
    var sendData = new BaseModule('oYCwTt0S4M_WUvVunRwdmEnWaUn0','dO_7xn28VvMKJRDmiReTY-8xR122RW2WodDurAF5zcs', data);

    needle.post('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=' + token, JSON.stringify(sendData), {}, function(err, resp, body) {
        if (err) {
            console.log(err);
        }
        console.log(body);
...
/*
 <xml><ToUserName><![CDATA[gh_4fc42d3fbd80]]></ToUserName>
 <FromUserName><![CDATA[oYCwTt0S4M_WUvVunRwdmEnWaUn0]]></FromUserName>
 <CreateTime>1431608951</CreateTime>
...
function sendMenu() {
    getToken(function(err, token) {
        ...
        needle.post('https://api.weixin.qq.com/cgi-bin/menu/create?access_token=' + token, postData, {}, function(err, resp, body) {
            if (err) {
                console.log(err);
            }
            console.log(body);
