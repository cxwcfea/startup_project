...
var getServiceFee = function(apply, period) {
    ...
    if (apply.type && apply.type === 2) {
        return (apply.amount - (apply.amount / apply.lever)) * apply.interestRate * discount;
    }
    ...
    async.waterfall([
        ...
        function(apply, callback) { // for deposit output
            var orderData = {
                ...
                status: 2,
                description: '缴纳配资保证金 ' + apply.serialID,
                applySerialID: apply.serialID
            };
            Order.create(orderData, function(err, order) {
                if (!err && !order) {
    ...
    async.waterfall([
        ...
        function(referer, callback) {
            var data = {
                ...
                amount: income,
                status: 1,
                description: '用户推广配资收入7%返佣 ' + applySerialID,
        ...
        */
        ...
        for (var i = 0; i < todayAddedPayApplyData.length; ++i) {
            if (todayAddedPayApplyData[i].type === 2) {
                fee += util.getServiceFee(todayAddedPayApplyData[i]) / 30;
                continue;
            }
