...
var getServiceFee = function(apply, period) {
    ...
    return apply.amount / 10000 * getServiceCharge(apply.lever) * period;
};
...
module.exports.getPayUserNum = function(cb) {
    Apply.find({ $and: [{isTrial:false}, {status:{$ne:1}}, {status:{$ne:9}}] }, function(err, applies) {
        if (err) {
            logger.debug('err when getPayUserNum ' + err.toString());
            return cb(err);
        }
        var count = 0;
        var amount = 0;
...
module.exports.getPayUserNumOfToday = function(cb) {
    var startTime = moment().startOf('day');
    Apply.find({ $and: [{applyAt: {$gte: startTime.toDate()}}, {status:2}] }, function(err, applies) {
        var nameMap = {};
        for (var i = 0; i < applies.length; ++i) {
            if (!nameMap[applies[i].userMobile]) {
                nameMap[applies[i].userMobile] = true;
