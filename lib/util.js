...
var sendInvestSuccessSMS = function(mobile, amount, total_amount, rate) {
    if (!mobile) {
...
module.exports.sendSMS_14 = sendInvestSuccessSMS;

var sendReturnProfitSMS = function(mobile, amount, total_amount, rate, date, profit) {
    if (!mobile) {
...
module.exports.sendSMS_15 = sendReturnProfitSMS;
...
module.exports.applyConfirmed = function(user, apply, cb) {
    async.waterfall([
        ...
        function(callback) {
            var period = moment(apply.endTime).dayOfYear() - moment(apply.startTime).dayOfYear() + 1;
            findInvestorForApply(apply, period, function(err) {
                callback(err);
            });
        }
    ], function(err) {
...
module.exports.getUserViewModel = function (user, cb) {
    ...
    Investor.findOne({userID:user._id}, function(err, investor) {
        if (err) {
            logger.error('getUserViewModel error when find investor ' + err.toString());
        }
        var ret;
        if (investor) {
            ret = _.extend(vm, {availableInvestAmount:investor.amount});
        } else {
            ret = _.extend(vm, {availableInvestAmount:0});
        }
        cb(ret);
    });
};
...
module.exports.getApplyData = function(cb) {
    ...
    Apply.find({}, function(err, applies) {
        ...
        var todayAddedFreeApplyData = applies.filter(function(elem) {
            return elem.startTime <= endTime && elem.startTime >= startTime && elem.isTrial === true && elem.status === 2;
        });
        ...
        for (var i = 0; i < todayAddedPayApplyData.length; ++i) {
            ...
            if (todayAddedPayApplyData[i].serviceCharge) {
                fee += todayAddedPayApplyData[i].serviceCharge * todayAddedPayApplyData[i].amount / 10000;
            } else {
                switch (todayAddedPayApplyData[i].lever) {
                    case 10:
                        fee += 19.9 * todayAddedPayApplyData[i].amount / 10000;
                        break;
                    case 9:
                        fee += 18.9 * todayAddedPayApplyData[i].amount / 10000;
                        break;
                    case 8:
                        fee += 17.9 * todayAddedPayApplyData[i].amount / 10000;
                        break;
                    case 7:
                        fee += 16.9 * todayAddedPayApplyData[i].amount / 10000;
                        break;
                    case 6:
                        fee += 15.9 * todayAddedPayApplyData[i].amount / 10000;
                        break;
                    case 5:
                        fee += 10.9 * todayAddedPayApplyData[i].amount / 10000;
                        break;
                    default :
                        fee += 19.9 * todayAddedPayApplyData[i].amount / 10000;
                        break;
    ...
    async.waterfall([
        ...
        function(user, order, callback) {
            if (user) {
                ...
                orderFinished(user, order, 1, function(err) {
                    if (!err) {
                        sendReturnProfitSMS(user.mobile, investAmount, data.contract.amount, order.profitRate, moment(data.contract.startTime).format('MMDD'), order.amount);
                    }
                    callback(err);
                })
            } else {
...
var returnProfitToInvestor = function(contract, cb) {
    ...
    async.waterfall([
        ...
        function(apply, callback) {
            ...
            if (contractEndDay < applyEndDay && apply.status === 2) {
                findInvestorForApply(apply, applyEndDay - contractEndDay, function(err) {
                    callback(err);
                });
            } else {
                callback(null);
...
module.exports.returnProfitToInvestor = returnProfitToInvestor;
            ...
            Order.create(orderData, function(err, order) {
                ...
                if (err) {
                    ...
                    User.update({mobile:order.userMobile}, {$inc: {'finance.history_invest_amount':order.amount}}, function(err, numberAffected, raw) {
                        sendInvestSuccessSMS(order.userMobile, order.amount, data.contract.amount, order.profitRate);
                        callback(err);
                    });
                }
            });
...
var findInvestorForApply = function(apply, period, cb) {
