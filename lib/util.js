...
var getEndDay = function(startDay, period, type) {
    if (type && type === 2) {
        var endDay = moment().add(period, 'months');
...
var getServiceFee = function(amount, period) {
    ...
    async.waterfall([
        function(callback) {
            if (apply.status > 1) {
                ...
                apply.endTime = getEndDay(startDay, apply.period).toDate();
                apply.status = 4;
        ...
        function(user, callback) {
            var serviceFee = getServiceFee(apply.amount, apply.period);
            if (!apply.isTrial) {
                var orderData = {
                    userID: user._id,
                    userMobile: user.mobile,
    ...
    async.waterfall([
        ...
        function(callback) {
            if (!apply.isTrial) {
                ...
                var totalServiceFee = getServiceFee(apply.amount, apply.period);
                var actualServiceFee = getServiceFee(apply.amount, tradeDays);
