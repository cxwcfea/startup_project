...
var sms_macro = [
    {
        ...
        value: 0,
        content: '您于TIME在牛金网提现AMOUNT元，提现已完成，请注意查收。'
    },
    {
    ...
    },
    {
        ...
        value: 2,
        content: '您有一笔金额为AMOUNT元配资资金已到账，交易账号ACCOUNT，登录密码PASSWORD，您可以通过手机或电脑进行操盘，可以登录牛金网，在我的账户中查看相应账户信息，操盘账户是您操盘的唯一依据，请不要向任何人泄露！'
    },
    {
        name: 'close_apply_sms_content',
        value: 3,
        content: '您在牛金网有一笔TOTAL_AMOUNT元的配资业务已经结算完毕，根据您的操盘收益，该笔配资实际结算AMOUNT元（保证金DEPOSIT元，收益PROFIT元），祝您投资愉快。'
    },
    {
...
module.exports.sendSMS_3 = function(mobile, total_amount, amount, deposit, profit) {
    var message_content = sms_macro[3].content.replace('TOTAL_AMOUNT', total_amount).replace('AMOUNT', amount).replace('DEPOSIT', deposit).replace('PROFIT', profit);
    sms.sendSMS(mobile, '', message_content, function (result) {
        if (result.error) {
            return {error_code:1, error_msg:result.msg};
        } else {
