...
function saveBackInvestor(data, cb) {
    async.waterfall([
        ...
        function (order, callback) {
            ...
            //weixin.sendWeixinTemplateMsg(order.userMobile, {t_id:4, type:'投资成功', amount:order.amount});
            callback(null);
        }
    ], function (err) {
        if (err) {
...
function returnProfitForEachInvestor(data, callback) {
    async.waterfall([
        ...
        function(user, profitOrder, returnOrder, callback) {
            ...
            //var investSpan = moment(data.contract.startTime).format('M月DD日')+'-'+moment(data.contract.endTime).format('M月DD日');
            //var fee = '本金'+data.investor.amount+'元'+'+'+'利息'+profitOrder.amount+'元';
            //weixin.sendWeixinTemplateMsg(user.mobile, {t_id:7, type:'投资结算成功', amount:fee, date:investSpan});
...
module.exports.findInvestorForApply = function(apply, cb) {
    logger.debug('findInvestorForApply:' + apply.serialID);
    if (apply.isTrial || apply.type === 2) {
        logger.info('findInvestorForApply free or yyn not involve invest:' + apply.serialID);
       return cb(null);
    }
    if (!apply.userName || !apply.userIdentity) {
        logger.warn('findInvestorForApply warn apply user no identity:' + apply.serialID);
        return cb(null);
    }
    var queue = new priorityQueue(function(a, b) {  // a, b should be user instance
        if (a.invest.profitRate < b.invest.profitRate) {
            ...
            // REVIEW(tianjia): should be time-prioritized, not amount
    ...
    async.waterfall([
        function(callback) {
            var data = {
                ...
                userIdentity: apply.userIdentity,
                amount: apply.amount,
        ...
        function(contract, callback) {
            User.find({$and:[{'invest.enable':true}, {'invest.duration':{$gte:contract.period}}, {'invest.availableAmount':{$gte:100}}]}, function(err, investors) {
        ...
        function(contract, callback) {
            ...
            while (amount >= 100 && queue.size()) {
                ...
                if (apply.userMobile == investor.mobile) {
                    continue;
                }
                var pay;
        ...
        if (err) {
            logger.warn('findInvestorForApply error:' + err.toString() + ' ' + apply.serialID);
        }
        cb(null);
    });
};
