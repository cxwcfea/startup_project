'use strict';
angular.module('supportApp').controller('SupportMyApplyListCtrl', ['$scope', '$http', '$location', '$modal', 'gbNotifier', 'days', 'util', '$filter', '$window', function($scope, $http, $location, $modal, gbNotifier, days, util, $filter, $window) {
    var vm = this;
    var apply_list = {};
    var currentApplies;
    vm.itemsPerPage = 15;
    ...
    function initData() {
        $http.get('/admin/api/applies/all').success(function(data) {
            ...
            var new_data = [];
            for (var key in data) {
              if (data[key].manager == vm.user.mobile) {
            ...
            currentApplies = $filter('orderBy')(apply_list, 'createdAt', true);
            pageReset();
        });
    }
    ...
    vm.takeApply = function(apply) {
        $http.post('/admin/api/take_apply', {id:apply._id})
