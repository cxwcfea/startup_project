...
angular.module('mobileApp').controller('MobileApplyDetailCtrl', ['$scope', '$window', '$location', '$http', '$timeout', 'days', 'util', function($scope, $window, $location, $http, $timeout, days, util) {
    ...
    if (!vm.user) {
        ...
        vm.apply_warn = vm.apply.isTrial ? 1900 : (vm.apply.warnValue ? vm.apply.warnValue : util.getWarnValue(vm.apply.amount, vm.apply.deposit));
        vm.apply_sell = vm.apply.isTrial ? 1800 : (vm.apply.sellValue ? vm.apply.sellValue : util.getSellValue(vm.apply.amount, vm.apply.deposit));
    }
